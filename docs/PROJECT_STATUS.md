...
| Market Scanner | 50-symbol watchlists, additive manual/AI discovery with explicit review, checkbox-based removals, manual and five-minute scheduled scans, real Alpaca evidence, live session open/close countdown timer, closed-market guidance banner, no synthetic fallback, and tenant-scoped scan-run history are implemented. |
